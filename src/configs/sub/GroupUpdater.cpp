...
struct ProviderMeta {
    ...
    void read(const QString &key, const QString &value) {
        if (key == "profile-title") {
            ...
            if (announce.isEmpty()) announce = decodeProviderValue(value);
        } else if (key == "support-url") {
            if (supportUrl.isEmpty()) supportUrl = sanitizeProviderUrl(value);
        } else if (key == "profile-web-page-url") {
            if (webPageUrl.isEmpty()) webPageUrl = sanitizeProviderUrl(value);
        } else if (key == "fallback-url") {
            if (fallbackUrl.isEmpty()) fallbackUrl = sanitizeProviderUrl(value);
...
void GroupUpdater::refresh(int gid, bool showDiff) {
    ...
    if (settings->sub_clear && !members().isEmpty() && !hasUsableProfile) {
        MW_show_log("<<<<<<<< " + QObject::tr(
                                      ...
                                      "group were kept. Use Clear servers if it really is empty now.")
                                      .arg(group->name));
        MW_dialog_message(MwMessage::SubscriptionFinished, {MwArg::Quiet});
        return;
    }
