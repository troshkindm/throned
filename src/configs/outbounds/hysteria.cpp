...
bool hysteria::ParseFromJson(const QJsonObject &object) {
    ...
    if (protocol_version == "1") {
        ...
        if (object.contains("obfs")) {
            ...
            if (obfsObj.contains("type"))
                obfs_type = obfsObj["type"].toString();
            else
                obfs_type = "salamander";
        ...
        if (object["realm"].isObject()) {
            ...
            if (realmObj["stun_servers"].isArray())
                realm_stun_servers = QJsonArray2QListString(realmObj["stun_servers"].toArray());
            else if (realmObj.contains("stun_servers"))
                realm_stun_servers = {realmObj["stun_servers"].toString()};
