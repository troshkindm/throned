...
DiagnosticsWindow::Usage MainWindow::diagnosticsUsage(int rangeDays) {
    ...
    usage.recording = !Configs::dataManager->settingsRepo->disable_traffic_stats && !Configs::dataManager->settingsRepo->disable_traffic_aggregation;
    ...
    for (const auto &row: repo->QueryConfigUsage(from, now)) {
        ...
        if (name.isEmpty()) {
            if (row.profile_id == Stats::DIRECT_STAT_PROFILE_ID)
                name = DiagnosticsWindow::tr("Direct");
            else if (const auto profile = Configs::dataManager->profilesRepo->GetProfile(row.profile_id))
                name = profile->name;
            else if (row.profile_id == Configs::warpProfileID)
                name = QStringLiteral("built-in warp");
            else
                name = DiagnosticsWindow::tr("Profile #%1 (deleted)").arg(row.profile_id);
        }
...
void MainWindow::openDiagnosticsTarget(const QString &target) {
    if (target == QLatin1String("rules")) {
        on_menu_routing_settings_triggered();
        return;
    }
    if (target == QLatin1String("log")) {
        openLogSettings();
        return;
    }
    if (target == QLatin1String("reachability")) {
    ...
    if (!diagnosticsWindow) {
        ...
        connect(window, &DiagnosticsWindow::healthRequested, window, [this, window] {
            ...
            background(window, [] {
                ...
                return std::make_pair(result, ok ? QString() : error); }, [window](const auto &result) { window->applyHealth(result.first, result.second); });
        });
        connect(window, &DiagnosticsWindow::routeRequested, window, [window](const libcore::PreviewRouteRequest &request) {
            background(window, [request] {
                ...
                if (!ok && error.isEmpty()) error = tr("The core is unavailable. Start a connection and try again.");
                return std::make_pair(result, ok ? QString() : error); }, [window](const auto &result) { window->applyRoutePreview(result.first, result.second); });
        });
        connect(window, &DiagnosticsWindow::siteRequested, window, [window](const libcore::DiagnoseSiteRequest &request) {
            background(window, [request] {
                ...
                if (!ok && error.isEmpty()) error = tr("The core is unavailable. Start a connection and try again.");
                return std::make_pair(result, error); }, [window](const auto &result) { window->applySiteResult(result.first, result.second); });
        });
        connect(window, &DiagnosticsWindow::matrixRequested, window, [window](const QString &url, const QStringList &outbounds) {
            for (const auto &tag: outbounds) {
                background(window, [url, tag] {
                    ...
                    libcore::DiagnoseSiteResponse result;
                    if (API::defaultClient) result = API::defaultClient->DiagnoseSite(&ok, request, &error);
                    return std::make_tuple(tag, result, ok ? QString() : error); }, [window](const auto &result) { window->applyMatrixResult(std::get<0>(result), std::get<1>(result), std::get<2>(result)); });
            }
        });
        connect(window, &DiagnosticsWindow::connectionsRequested, window, [window] {
            background(window, [] {
                ...
                return std::make_pair(result, ok ? QString() : tr("The core is unavailable.")); }, [window](const auto &result) { window->applyConnections(result.first, result.second); });
        });
