...
QList<std::shared_ptr<RouteRule>> RouteProfile::get_simple_rules() {
    ...
    static constexpr std::array kConfigs = {
        ...
        RuleConfig{simpleAddressViaProfile, u"route", proxyID},
        RuleConfig{simpleProcessNameViaProfile, u"route", proxyID},
        RuleConfig{simpleProcessPathViaProfile, u"route", proxyID},
    };
...
RouteProfile::ProcessSelectors RouteProfile::get_process_selectors(int outbound) const {
    ...
    for (const auto& item: Rules) {
        ...
        if (item->process_name.isEmpty() && item->process_path.isEmpty() && item->process_path_regex.isEmpty())
            continue;
        const bool onlyProcess = item->ip_version.isEmpty() && item->network.isEmpty() && item->protocol.isEmpty() && item->inbound.isEmpty() && item->domain.isEmpty() && item->domain_suffix.isEmpty() && item->domain_keyword.isEmpty() && item->domain_regex.isEmpty() && item->source_ip_cidr.isEmpty() && item->ip_cidr.isEmpty() && item->source_port.isEmpty() && item->source_port_range.isEmpty() && item->port.isEmpty() && item->port_range.isEmpty() && item->wifi_ssid.isEmpty() && item->wifi_bssid.isEmpty() && item->rule_set.isEmpty() && !item->source_ip_is_private && !item->ip_is_private && !item->invert;
...
bool RouteProfile::UsesProcessRules() const {
    for (const auto& rule: Rules) {
        if (!rule) continue;
        if (!rule->process_name.isEmpty() || !rule->process_path.isEmpty() || !rule->process_path_regex.isEmpty())
            return true;
    }
    return false;
}
...
QList<ruleType> RouteProfile::simple_rule_types(simpleAction action) {
    switch (action) {
        case proxy:
            return {simpleAddressProxy, simpleProcessNameProxy, simpleProcessPathProxy};
        case bypass:
            return {simpleAddressBypass, simpleProcessNameBypass, simpleProcessPathBypass};
        case warpBypass:
            return {simpleAddressWarpBypass, simpleProcessNameWarpBypass, simpleProcessPathWarpBypass};
        case viaProfile:
            return {simpleAddressViaProfile, simpleProcessNameViaProfile, simpleProcessPathViaProfile};
        default:
            return {simpleAddressBlock, simpleProcessNameBlock, simpleProcessPathBlock};
    }
}
...
void RouteProfile::RemoveSimpleViaProfile(int profileID) {
    ...
    QList<std::shared_ptr<RouteRule>> kept;
    for (const auto& item: Rules) {
        if (item != nullptr && types.contains(static_cast<ruleType>(item->type)) && item->outboundID == profileID)
            continue;
        kept << item;
    }
    Rules = kept;
...
bool RouteProfile::add_simple_rule(const QString& content, const std::shared_ptr<RouteRule>& rule, ruleType type) {
    if (type == simpleAddressProxy || type == simpleAddressBypass || type == simpleAddressBlock || type == simpleAddressWarpBypass || type == simpleAddressViaProfile)
        ...
        return add_simple_process_rule(content, rule);
}
